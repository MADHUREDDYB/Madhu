...
class CustomQueryGenerator:
    ...
    def generate_queries(self,query_template_file):
        ...
        for data in self.sttm_data:
            ...
            for i in range(len(tgt_cols)):
                if src_cols[i] != 'NA':
                    if trnsfrms[i].replace(" ",'').upper() != 'DIRECTMAPPING':
                        query_src_cols.append(src_cols[i])
                        query_trnfrms.append(trnsfrms[i])
                        query_tgt_cols.append(tgt_cols[i])
                    else:
                        query_src_cols.append(src_cols[i])
                        query_trnfrms.append(src_cols[i] + ' AS ' + tgt_cols[i])
                        query_tgt_cols.append(tgt_cols[i])
                elif 'AUD' in src_cols[i].upper() or 'DLTD' in src_cols[i].upper():
                    aud_cols.append(tgt_cols[i])
                else:
                    aud_cols.append(tgt_cols[i])
            ...
            btch_nm = data[14]
            temp_col = data[15]
            ws1 = wb_output.create_sheet(title='%s' % src_tb_nm)
            ...
            if src_fltr is None or src_fltr.strip() == '':
                src_fltr = ''
            else:
                src_fltr = ' ' + src_fltr
            if src_join is None or src_fltr.strip() == '':
                src_join = ' '
            else:
                src_join = ' ' + src_join
            if tgt_fltr is None or tgt_fltr.strip() == '':
                tgt_fltr = ' '
            else:
                tgt_fltr = ' ' + tgt_fltr
            if tgt_join is None or tgt_join.strip() == '':
                tgt_join = ' '
            else:
                tgt_join = ' ' + tgt_join
            if temp_col is None or temp_col.strip() == '':
                temp_col = ''
            else:
                temp_col = ',' + temp_col
            ...
            for query_row in range(2,queries_template_sheet.max_row+1):
                ...
                if True:
                    if level.upper() == 'TABLE LEVEL':
                        query = self.DataReplace(query, query_src_cols, query_tgt_cols, src_db, tgt_db, src_tb_nm, tgt_tb_nm,query_trnfrms,src_fltr,src_join,tgt_fltr,tgt_join,mpng,btch_nm,temp_col,aud_cols, 'TABLELEVEL')
                        ...
                        query_num = 0
                        for i in range(len(query_tgt_cols)):
                            query1 = self.DataReplace(query, query_src_cols, query_tgt_cols, src_db, tgt_db, src_tb_nm, tgt_tb_nm,query_trnfrms, src_fltr,src_join,tgt_fltr,tgt_join,mpng,btch_nm,temp_col,aud_cols,query_num)
                            ws1.cell(row=row_num, column=1).value = ts_id
                            ws1.cell(row=row_num, column=2).value = "TC-" + str(row_num-1)
                            ws1.cell(row=row_num, column=3).value = tgt_db + '.' + tgt_tb_nm
                            ws1.cell(row=row_num, column=4).value = query_tgt_cols[i]
        ...
        final_folder = folder + '\\Custom Testing\\Generated_ETL_Queries'
        if not os.path.exists(final_folder):
            os.makedirs(final_folder)
        wb_output.save(final_folder+'\\%s-%s_%s.xlsx'%(src_db,tgt_db,self.run_time))
        return final_folder

    def DataReplace(self, query, src_cols, tgt_cols, src_db, tgt_db, src_tb_nm, tgt_tb_nm,trnsfrmns, src_fltr,src_join,tgt_fltr,tgt_join,job_nm,btch_nm,tempcol,aud_cols,flag):

        temp = ','.join(src_cols)
        if flag == 'TABLELEVEL':
            query1 = query.replace('<TGT_COL_NM_ALL>', ','.join(tgt_cols)).replace('<SRC_COL_NM_ALL>',temp).replace('<TGT_FLTR>', tgt_fltr).replace(
                '<TGT_JOIN>', tgt_join).replace('<DTA_TYP>', 'DATA_TYPE').replace('<SRC_TBL>', src_tb_nm).replace('<TRNS_ALL>',','.join(trnsfrmns)).replace('<SRC_FLTR>',src_fltr).replace('<SRC_JOIN>',src_join).replace('<SRC_DB>',src_db).replace('<TGT_TBL>',tgt_tb_nm).replace('<TGT_DB>',tgt_db).replace('<TEMP_COL>',tempcol).replace('<AUD_COL>',','.join(aud_cols))
        else:
            query1 = query.replace('<TGT_COL_NM>', tgt_cols[flag]).replace('<TGT_FLTR>', tgt_fltr).replace(
                ...
                '<SRC_COL_NM>', src_cols[flag]).replace('<SRC_FLTR>',src_fltr).replace('<SRC_JOIN>',src_join).replace('<TRNS>',trnsfrmns[flag]).replace('<SRC_DB>',src_db).replace('<TGT_TBL>',tgt_tb_nm).replace('<TGT_DB>',tgt_db).replace('<TEMP_COL>',tempcol).replace('<TRNS_ALL>',','.join(trnsfrmns)).replace('<AUD_COL>',','.join(aud_cols))
