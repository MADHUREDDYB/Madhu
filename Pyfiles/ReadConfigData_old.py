...
class ReadConfigData:
    ...
    #     queries_sheet = wb['Query']
    #     exe_sheet = wb["Execute"]
    #     queries_sheet_max_rows = queries_sheet.max_row
    #     exe_sheet_max_rows = exe_sheet.max_row
    #     queries_data = {}
    #     exe_data = {}
    #     for i in range(2, queries_sheet_max_rows+1):
    #         tst_cs_desc = queries_sheet.cell(row=i, column=1).value
    #         query = queries_sheet.cell(row=i, column=2).value
    #         queries_data[tst_cs_desc] = query
    #
    #     for i in range(2, exe_sheet_max_rows+1):
    #         tst_cs_desc = exe_sheet.cell(row=i, column=1).value
    #         exe_sts = exe_sheet.cell(row=i, column=2).value
    #         exe_data[tst_cs_desc] = exe_sts
    #
    #     return queries_data, exe_data
    ...
    def ReadSttmData(self,filename):
        ...
        for sheet1 in sheets:
            ...
            btch_nm = sheet.cell(row=3, column=4).value
            ...
            for i in range(11, sheet.max_row):
                ...
                if temp_src_col is not None and temp_src_col != 'NA':
                    src_cols.append(temp_src_col)
                if temp_tgt_col is not None and temp_tgt_col != 'NA':
                    tgt_cols.append(temp_tgt_col)
                if trnsfrmtns_temp is not None and trnsfrmtns_temp != 'NA':
                    trnsfrmtns.append(trnsfrmtns_temp)
                if temp_tgt_dttype is not None and temp_tgt_dttype != 'NA':
            ...
            sttm_data.append([src_cols, tgt_cols, src_db, tgt_db, src_tb_name, tgt_tb_name, tgt_mpng, tgt_workflow,trnsfrmtns,src_fltr,src_join,tgt_join,tgt_fltr,tgt_data_typs,btch_nm])
