...
import Pyfiles.ReadConfigData as rdconfig, Pyfiles.QueryGenerator as QG, Pyfiles.Databricks_conn as dbricks, \
    ...
    Pyfiles.JiraGenerator as JIRA, \
    Pyfiles.Defect_Tracker as DT, Pyfiles.Mail as Mail, Pyfiles.File_Compare as FILE_CMP, Pyfiles.STTM_VALIDATION as STTM_VAL,\
    Pyfiles.LoadStrategy as LD_STG, Pyfiles.CustomQueryGenerator as CSTM_QG,Pyfiles.CustomETLValidation as CSTM_ETL
...
app.layout = html.Div([
    html.Div([
        ...
        html.Div([
            ...
            html.A(html.Button('Clear', id='refresh_button', n_clicks=0,
                               style={'width': '10%', 'display': 'inline-block'}), href='/')]),
        dcc.Upload(
            id='upload-file',
        ...
        html.Div([
            ...
            html.Button('Generate Defects', id='create_defect_tracker', n_clicks=0,
                        style={'width': '22.5%', 'display': 'inline-block', 'margin'
                        : '20px'}),
            html.Button('Upload Defects', id='upload_defect_tracker', n_clicks=0,
                        style={'width': '22.5%', 'display': 'inline-block', 'margin'
                        : '10px'}),
            html.Button('Email Results', id='send_email_123', n_clicks=0,
                        style={'width': '22.5%', 'display': 'inline-block', 'margin'
                        : '10px'}),
            html.Button('Export Logs', id='export_button', n_clicks=0,
                        style={'width': '22.5%', 'display': 'inline-block', 'margin'
                        : '10px'}),
            # html.Hr(style={'borderColor':'black'})
        ],style={'border':'1px black','border-style': 'inset'}),
        html.Div(id='email_div'),
        html.Div([
            html.H5('Message Board', style={'width': '88.8%', 'display': 'inline-block'})
        ]),
        html.Div(id='file_comparision_1'),
        html.Div(id='file_comparision'),
        html.Div(id='sttm_validation_1'),
        html.Div(id='sttm_validation'),
        html.Div(id='custom_generate_1'),
        html.Div(id='custom_generate'),
        html.Div(id='custom_execute_1'),
        html.Div(id='custom_execute'),
        html.Div(id='strategy_validation_before_1'),
        html.Div(id='strategy_validation_before'),
        html.Div(id='strategy_validation_after_1'),
        html.Div(id='strategy_validation_after'),
        ...
        html.Div(id='defect_tracker_start'),
        html.Div(id='defect_tracker'),
        html.Div(id='defect_uploader_start'),
        html.Div(id='defect_uploader'),

        dcc.Store(id='sttm_validation_export_data_1'),
        dcc.Store(id='sttm_validation_export_data'),
        dcc.Store(id='file_comparision_export_data_1'),
        dcc.Store(id='file_comparision_export_data'),
        dcc.Store(id='first_comparision_file'),
        dcc.Store(id='second_comparision_file'),
        dcc.Store(id='stategy_validation_file'),
        dcc.Store(id='strategy_validation_before_export_data_1'),
        dcc.Store(id='strategy_validation_before_export_data'),
        dcc.Store(id='strategy_validation_after_export_data_1'),
        dcc.Store(id='strategy_validation_after_export_data'),
        dcc.Store(id='custom_sttm_file'),
        dcc.Store(id='custom_generate_export_data_1'),
        dcc.Store(id='custom_generate_export_data'),
        dcc.Store(id='custom_execute_export_data_1'),
        dcc.Store(id='custom_execute_export_data'),
        ...
        dcc.Store(id='defect_tracker_start__data'),
        dcc.Store(id='upload_tracker_start__data'),
        dcc.Store(id='upload_tracker____data'),
        dcc.Store(id='defect_tracker____data'),
...
@app.callback(
    Output('defect_tracker', 'children'),
    Output('defect_tracker____data', 'data'),
...
def create_defect_tracker_function(n_clicks, config_data):
    if n_clicks > 0:
        ...
        c.createdefecttracker(config_data['JIRA Project Key'], config_data['JIRA Reporter'],
                              config_data['JIRA Assignee'])
        _val = 'Defect Tracker Created Successfully.'
        return (html.Div([html.Iframe(id="dt_md",
                                      srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                      style={'border': 0, 'width': '100%', 'height': (len(_val.split('<BR>')) * 25) + (
                                                  math.ceil(len(_val) / 170) * 25)})]),
                                             f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}'
                                      )
    return (None,None)
@app.callback(
    Output('defect_uploader', 'children'),
    Output('upload_tracker____data', 'data'),
...
def upload_defect_tracker_function(n_clicks, config_data):
    if n_clicks > 0:
        ...
        _val = 'Defect Tracker Uploaded Successfully.'
        return (html.Div([html.Iframe(id="du_md",
                                      srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                      style={'border': 0, 'width': '100%', 'height': (len(_val.split('<BR>')) * 25) + (
                                                  math.ceil(len(_val) / 170) * 25)})]),
                                             f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}'
...
@app.callback(
    ...
    Input('send_email_123', 'n_clicks'))
def send_email_function(n_clicks):
    if n_clicks > 0:
        #
        return (dcc.Upload(
            id='upload-file-email',
            children=html.Div(["Select E-Mail List"]
                              ),
            style={
                "width": "98%", "height": "60px", "lineHeight": "60px", "borderWidth": "1px", "borderStyle": "dashed",
                "borderRadius": "5px", "textAlign": "center", "margin": "10px",
...
@app.callback(
    ...
    State('config_input', 'data'))
def fetch_email_list(contents,config_dict):
    # ------------------------------------------------------------------------------------------------
    ...
    if contents != None:
        ...
        Exe_mail = Mail.Mailer(config_dict['Input folder path'],config_dict['Output folder path'],config_dict['src_sys_name'])
        Exe_mail.send_email(decoded)
...
@app.callback(
    Output('defect_tracker_start', 'children'),
    Output('defect_tracker_start__data', 'data'),
    Input('create_defect_tracker', 'n_clicks'),
    State('defect_tracker_start__data', 'data'))
def update_dashboard_message_board_2(n_clicks,already_out):
    if n_clicks > 0:
        children_list = []
        _val = 'Creating Defect Tracker'
        children_list.append(html.Div([html.Iframe(id="defect_tracker_start__1",
                                                   srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                   style={'border': 0, 'width': '100%',
                                                          'height': (len(_val.split('<BR>')) * 25) + (
                                                                      math.ceil(len(_val) / 170) * 25)}
                                                   )]))
        if already_out != None:
            return (children_list,
                    str(already_out)+'None' + f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}')
        else:
            return (children_list,
                    str(already_out)+'None' + f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}')
    if already_out not in [None,'','None']:
        already_out_children = html.Div([html.Iframe(
                            srcDoc=f"{already_out.split('None')[-1]}",
                            style={'border': 0, 'width': '100%',
                                    'height': (len(already_out.split('None')[-1].split('<BR>')) * 15) + (
                                                math.ceil(len(already_out.split('None')[-1]) / 170) * 25)}
                            )])
    else:
        already_out_children = None
    already_out = 'None' + already_out
    return (already_out_children,already_out)
@app.callback(
    Output('defect_uploader_start', 'children'),
    Output('upload_tracker_start__data', 'data'),
    Input('upload_defect_tracker', 'n_clicks'),
    State('upload_tracker_start__data', 'data'))
def update_dashboard_message_board_2(n_clicks,already_out):
    if n_clicks > 0:
        _val = 'Uploading Defect Tracker'
        children_list = []
        children_list.append(html.Div([html.Iframe(id="defect_uploader_start__1",
                                                   srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                   style={'border': 0, 'width': '100%',
                                                          'height': (len(_val.split('<BR>')) * 25) + (
                                                                      math.ceil(len(_val) / 170) * 25)}
...
@app.callback(
    ...
    State('file_comparision_export_data_1','data'),
    State('file_comparision_export_data','data'),
    State('sttm_validation_export_data_1','data'),
    State('sttm_validation_export_data','data'),
    State('strategy_validation_before_export_data_1','data'),
    State('strategy_validation_before_export_data','data'),
    State('strategy_validation_after_export_data_1','data'),
    State('strategy_validation_after_export_data','data'),
    State('custom_generate_export_data_1','data'),
    State('custom_generate_export_data','data'),
    State('custom_execute_export_data_1','data'),
    State('custom_execute_export_data','data'),
    ...
    State('defect_tracker_start__data', 'data'),
    State('defect_tracker____data', 'data'),
    State('upload_tracker_start__data', 'data'),
    State('upload_tracker____data', 'data'),
    )
def export_message_board(n_clicks, config_dict, q, q1, r, r1, s1, s2, s3, s4, t1, t2, t3, t4, a1, a, b1, b, c1, c, d1, d, e1, e, f1, f, g1, g, h1, h, i1, i, j1, j,
                         k1, k, l1, l, m1, m, n1, n, o1, o, p1, p,):
    # ------------------------------------------------------------------------------------------------
    ...
    if n_clicks > 0:
        _logs_to_store = []
        for __log_from_store in [q, q1, r, r1, s1, s2, s3, s4,t1, t2, t3, t4, a1, a, b1, b, c1, c, d1, d, e1, e, f1, f, g1, g, h1, h, i1, i, j1, j, k1, k, l1, l, m1,
                                 m, n1, n, o1, o, p1, p,]:
            if __log_from_store != None:
                # __log_from_store = __log_from_store.replace('None', '')
                __log_from_store = __log_from_store.split('None')[-1]
...
@app.callback(
    ...
    Output('upload-file','disable_click')],
...
def indicate_upload_file(_configdata):
    ...
    # ------------------------------------------------------------------------------------------------
    if _configdata != None:
        return (html.Div(["Configuration File Uploaded"])),True
    return (html.Div(["Upload Configuration File"])),False
    ...
    if _configdata != None:
        _l = [html.H6('Select an Option:'),
              dcc.RadioItems(id='level_1_generated', options=[
                  {'label': 'File Comparision', 'value': 'File Comparision'},
                  {'label': 'STTM Validation', 'value':'STTM Validation'},
                  {'label': 'Custom Testing', 'value':'Custom Testing'},
                  {'label': 'Load Strategy Validation', 'value': 'Strategy Validation'},
...
def update_dashboard_checklist_1(radio_selection):
    ...
    # ------------------------------------------------------------------------------------------------
    if radio_selection == 'Query Generation':
        _l = [html.Hr(),html.H6('Select from following Options:'),
              ...
              html.Button(children = 'Compare',
                                id='file_compare_button',
                                n_clicks=0,
                                style={'display':'none',
                                    }),

                html.Button(children = 'Compare',
                                id='strategy_before_job',
                                n_clicks=0,
                                style={'display':'none',
                                    }),
                html.Button(children = 'Compare',
                                id='strategy_after_job',
                                n_clicks=0,
                                style={'display':'none',
                                    }),
                html.Button(children = 'Compare',
                                id='custom_generate_query',
                                n_clicks=0,
                                style={'display':'none',
                                    }),
                html.Button(children = 'Compare',
                                id='custom_execute_query',
                                n_clicks=0,
                                style={'display':'none',
                                    })]
        return (_l)
    ...
    elif radio_selection == 'File Comparision':
        _l = [html.Hr(),
            html.H6('Upload Files for Comparision',style={'padding-left':'10px'}),
            html.Div([html.Div(dcc.Upload(
                        id='first-file-for-comparision',
                        children="Upload First File",
                        ...
                        disable_click = False,),style={'margin-right':'5px'}),

                    html.Div(dcc.Upload(
                            id='second-file-for-comparision',
                            children="Upload Second File",
                            style={"borderWidth": "1px",
                                    "borderStyle": "dashed",
                                    "borderRadius": "5px",
                                    "textAlign": "center",
                            ...
                            disable_click = False,),style={'margin-left':'5px','margin-right':'5px'},id='level_2_generated'),
            html.Div([html.Button(children = 'Compare',
                                id='file_compare_button',
                                n_clicks=0,
                                style={"borderWidth": "1px",
                                    "borderRadius": "5px",
    ...
    elif radio_selection == 'Strategy Validation':
        _l = [html.Hr(),
            html.H6('Load Strategy Validation'),
                html.Div(dcc.Upload(
                    id='strategy_file_upload',
                    children="Upload Load Details File",
                    style={"lineHeight": "60px", "borderWidth": "1px", "borderStyle": "dashed",
                        "borderRadius": "5px", "textAlign": "center", "margin": "10px",'width':'50%',
                    },
                    disable_click = False,),id='level_2_generated'),
                    html.Div([
            html.Button('Execution Before Job Run', id='strategy_before_job', n_clicks=0,
                        style={'width': '20%', 'display': 'inline-block', 'margin-left'
                        : '60px'}),
            html.Button('Execution After Job Run', id='strategy_after_job', n_clicks=0,
                        style={'width': '20%', 'display': 'inline-block', 'margin-left'
                        : '40px'})]),
                html.Button(children = 'Compare',
                                id='submit_2',
    ...
    elif radio_selection == 'STTM Validation':
        _l = [html.Hr(),html.Div(id='level_2_generated',style = {'display':'none'}),
                html.Button(children = 'Run STTM Validation',
                            id='sttm_validation_run_button',
                            n_clicks=0,style={'width': '22.5%', 'display': 'inline-block', 'margin': '20px'}),
                            html.Button(children = 'Compare',
                                id='submit_2',
                                n_clicks=0,
                                style={'display':'none',
    ...
    elif radio_selection == 'Custom Testing':
        _l = [html.Hr(),html.H6('Custom Testing'),
                html.Div(dcc.Upload(
                    id='custom_test_file_upload',
                    children="Upload Custom STTM File",
                    style={"lineHeight": "60px", "borderWidth": "1px", "borderStyle": "dashed",
                        "borderRadius": "5px", "textAlign": "center", "margin": "10px",'width':'50%',
                    },
                    disable_click = False,),id='level_2_generated'),
                    html.Div([
            html.Button('Generate Query', id='custom_generate_query', n_clicks=0,
                        style={'width': '22.5%', 'display': 'inline-block', 'margin'
                        : '20px'}),
            html.Button('Execute Query', id='custom_execute_query', n_clicks=0,
                        style={'width': '22.5%', 'display': 'inline-block', 'margin'
                        : '10px'})]) ,
                html.Button(children = 'Compare',
                                id='file_compare_button',
...
@app.callback(
    ...
    Input('submit_2', 'n_clicks'),Input('file_compare_button','n_clicks'),Input('strategy_before_job','n_clicks'),Input('strategy_before_job','n_clicks'),Input('custom_execute_query','n_clicks'),Input('custom_generate_query','n_clicks'),
    State('level_2_generated', 'value'),
    State('level_1_generated','value'))
def update_dashboard_checklist_2(n_clicks,a,b,c,d,e, selection_list, main_radio_selection):
    # ------------------------------------------------------------------------------------------------
    # Function generates checklists based upon values selected in checklist generated from previous function
    # ------------------------------------------------------------------------------------------------
    if n_clicks > 0 and main_radio_selection == 'Execution':
        _flag = 0
...
###File Comparision
@app.callback(
    [Output('first-file-for-comparision', 'children'),
    Output('first-file-for-comparision','disable_click'),
    Output('first_comparision_file','data')],
    [Input('first-file-for-comparision', 'contents'),
    Input('first-file-for-comparision', 'filename')])
def indicate_first_file_upload(_configdata,filename):
    # ------------------------------------------------------------------------------------------------
    # Function indicates whether input file is uploaded or not by changing the text within the textbox.
    # ------------------------------------------------------------------------------------------------
    if _configdata != None:
        return (html.Div(["First File Uploaded"])),True,filename
    return (html.Div(["Upload First File"])),False,None

@app.callback(
    [Output('second-file-for-comparision', 'children'),
    Output('second-file-for-comparision','disable_click'),
    Output('second_comparision_file','data')],
    [Input('second-file-for-comparision', 'contents'),
    Input('second-file-for-comparision', 'filename')])
def indicate_second_file_upload(_configdata,filename):
    # ------------------------------------------------------------------------------------------------
    # Function indicates whether input file is uploaded or not by changing the text within the textbox.
    # ------------------------------------------------------------------------------------------------
    if _configdata != None:
        return (html.Div(["Second File Uploaded"])),True,filename
    return (html.Div(["Upload Second File"])),False,None

@app.callback(
            [Output('file_comparision_1','children'),
            Output('file_comparision_export_data_1','data')],
            [Input('file_compare_button','n_clicks')],
            State('file_comparision_export_data_1','data'))
def update_dashboard_message_board_15(n_clicks,already_out):
    if n_clicks > 0:
        children_list = []
        _val = 'Started File Comparision'
        children_list.append(html.Div([html.Iframe(id="file_comparision_msg",
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
                                                                        math.ceil(len(_val) / 170) * 25)}
                                                    )]))
    ...
    return (already_out_children,already_out)
@app.callback(
            [Output('file_comparision','children'),
            Output('file_comparision_export_data','data')],
            [Input('file_compare_button','n_clicks')],
            [State('first_comparision_file','data'),
            State('second_comparision_file','data'),
            State('config_input','data'),
            State('file_comparision_export_data','data')])
def update_dashboard_message_board_15(n_clicks,file1,file2,config_dict,already_out):
    ##### Have given 2 files names as input with input location add in ut function here #########
    if n_clicks > 0:
        children_list = []

        file_path = config_dict['Input folder path']
        _val = f'function({file1},{file2},{file_path})'

        children_list.append(html.Div([html.Iframe(
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
                                                                        math.ceil(len(_val) / 170) * 25)}
        ...
        run_time = datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S").replace(" ", '-').replace(':',
                                                                                                   '').replace(
            '/', '')
        file_compare = FILE_CMP.FileCompare(run_time,config_dict['Input folder path'],config_dict['Output folder path'],config_dict['src_sys_name'],file1,file2)
        _val1 = file_compare.compare_files()
        if already_out != None:
            return (children_list,
                    str(already_out)+'None' + f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val1}')
        else:
            return (children_list,
                    str(already_out)+'None' + f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val1}')
    if already_out not in [None,'','None']:
...
#STTM Validation
@app.callback(
    Output('sttm_validation_1', 'children'),
    Output('sttm_validation_export_data_1','data'),
    Input('sttm_validation_run_button', 'n_clicks'),
    State('sttm_validation_export_data_1','data'))
def update_dashboard_message_board_16(n_clicks,already_out):
    if n_clicks > 0:
        children_list = []
        _val = 'Started STTM Validation'
        children_list.append(html.Div([html.Iframe(
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
    ...
    return (already_out_children,already_out)
@app.callback(
            Output('sttm_validation', 'children'),
            Output('sttm_validation_export_data','data'),
            Input('sttm_validation_run_button', 'n_clicks'),
            State('sttm_validation_export_data','data'),
            State('config_input','data'),)
def update_dashboard_message_board_16(n_clicks,already_out,config_dict):
    ### Add STTM Validation Function Here ######

    if n_clicks > 0:
        children_list = []
        _val = 'STTM Validation is Completed'
        run_time = datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S").replace(" ", '-').replace(':',
                                                                                                   '').replace(
            '/', '')
        validate_sttms = STTM_VAL.ValidateSTTM(run_time, config_dict['Input folder path'],
                                               config_dict['Output folder path'], config_dict['src_sys_name'])
        for file in [config_dict['Lake to Lake Hist STTM name'], config_dict['Lake Hist to Hub STTM name'],
                     config_dict['Hub to Mart STTM name']]:
            _val = validate_sttms.validate_lake_mart_sttm(file)
        if _val == '':
            _val = 'STTM Validation is Succesful'
        children_list.append(html.Div([html.Iframe(id="sttm_validation_msg_func",
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
                                                                        math.ceil(len(_val) / 170) * 25)}
    ...
    already_out = 'None' + already_out
    return (already_out_children,already_out)
# Load Stategy Validation
@app.callback(
    [Output('strategy_file_upload', 'children'),
    Output('strategy_file_upload','disable_click'),
    Output('stategy_validation_file','data')],
    [Input('strategy_file_upload', 'contents'),
    Input('strategy_file_upload', 'filename')])
def indicate_strategy_file_upload(_configdata,filename):
    # ------------------------------------------------------------------------------------------------
    # Function indicates whether input file is uploaded or not by changing the text within the textbox.
    # ------------------------------------------------------------------------------------------------
    if _configdata != None:
        return (html.Div(["Load Details File Uploaded"])),True,filename
    return (html.Div(["Upload Load Details File"])),False,None

@app.callback(
    [Output('strategy_validation_before_1', 'children'),
    Output('strategy_validation_before_export_data_1','data')],
    Input('strategy_before_job', 'n_clicks'),
    State('strategy_validation_before_export_data_1','data'))
def update_dashboard_message_board_17(n_clicks,already_out):
    if n_clicks > 0:
        children_list = []
        _val = 'Started Execution Before Run'
        children_list.append(html.Div([html.Iframe(id="exe_before_run_msg",
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
                                                                        math.ceil(len(_val) / 170) * 25)}
    ...
    return (already_out_children,already_out)
@app.callback(
            [Output('strategy_validation_before', 'children'),
            Output('strategy_validation_before_export_data','data')],
            Input('strategy_before_job', 'n_clicks'),
            State('stategy_validation_file','data'),
            State('config_input','data'),
            State('strategy_validation_before_export_data','data'))
def update_dashboard_message_board_17(n_clicks,file,config_dict,already_out):
    ### Add Strategy Validation Function Here ######
    ...
    if n_clicks > 0:
        ...
        conn = dbricks.DatabricksConnection.getConnection('', config_dict['Databricks connection string'])
        lg = LD_STG.LoadStrategy(file,config_dict['Input folder path'],config_dict['Output folder path'], run_time)
        lg.read_file('Before',conn)
        _val = f'function({file},{file_path})'
        _val = 'LoadStarategy Validation is completed Successfully'
        children_list.append(html.Div([html.Iframe(id="strategy_validation_msg_func",
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
                                                                        math.ceil(len(_val) / 170) * 25)}
    ...
    return (already_out_children,already_out)
@app.callback(
    [Output('strategy_validation_after_1', 'children'),
    Output('strategy_validation_after_export_data_1','data')],
    Input('strategy_after_job', 'n_clicks'),
    State('strategy_validation_after_export_data_1','data'))
def update_dashboard_message_board_17_1(n_clicks,already_out):
    if n_clicks > 0:
        children_list = []
        _val = 'Started Execution After Run'
        children_list.append(html.Div([html.Iframe(id="exe_before_run_msg",
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
    ...
    return (already_out_children,already_out)
@app.callback(
            [Output('strategy_validation_after', 'children'),
            Output('strategy_validation_after_export_data','data')],
            Input('strategy_after_job', 'n_clicks'),
            State('stategy_validation_file','data'),
            State('config_input','data'),
            State('strategy_validation_after_export_data','data'))
def update_dashboard_message_board_17_1(n_clicks,file,config_dict,already_out):
    ### Add Strategy Validation Function Here ######
...
# # Custom Testing
@app.callback(
    [Output('custom_test_file_upload', 'children'),
    Output('custom_test_file_upload','disable_click'),
    Output('custom_sttm_file','data')],
    [Input('custom_test_file_upload', 'contents'),
    Input('custom_test_file_upload', 'filename')])
def indicate_custom_sttm_file_upload(_configdata,filename):
    # ------------------------------------------------------------------------------------------------
    # Function indicates whether input file is uploaded or not by changing the text within the textbox.
    # ------------------------------------------------------------------------------------------------
    if _configdata != None:
        return (html.Div(["STTM File Uploaded"])),True,filename
    return (html.Div(["Upload Custom STTM File"])),False,None

@app.callback(
    [Output('custom_generate_1', 'children'),
    Output('custom_generate_export_data_1','data')],
    Input('custom_generate_query', 'n_clicks'),
    State('custom_generate_export_data_1','data'))
def update_dashboard_message_board_18(n_clicks,already_out):
    if n_clicks > 0:
        children_list = []
        _val = 'Started Custom Generating Query'
        children_list.append(html.Div([html.Iframe(id="custom_generate_msg",
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
                                                                        math.ceil(len(_val) / 170) * 25)}
...
@app.callback(
            [Output('custom_generate', 'children'),
            Output('custom_generate_export_data','data')],
            Input('custom_generate_query', 'n_clicks'),
            State('custom_sttm_file','data'),
            State('config_input','data'),
            State('custom_generate_export_data','data'))
def update_dashboard_message_board_18(n_clicks,file,config_dict,already_out):
    ### Add Custom Testing Function Here ######
    ...
    if n_clicks > 0:
        ...
        sttm_data = rdconfig.ReadConfigData.ReadSttmData("",
                                                         config_dict['Input folder path'] + '\\Input Files\\' +
                                                         file)
        custom_query_gen = CSTM_QG.CustomQueryGenerator(sttm_data, config_dict['Output folder path'], run_time)
        _val = custom_query_gen.generate_queries(config_dict['Input folder path'] + '\\Pyfiles\\Custom_Query_tempelate.xlsx')
        _val = "Custom Query Generation Completed Successfully, Queries placed at : %s" % _val
        children_list.append(html.Div([html.Iframe(id="strategy_validation_msg_func",
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
...
@app.callback(
    [Output('custom_execute_1', 'children'),
    Output('custom_execute_export_data_1','data')],
    Input('custom_execute_query', 'n_clicks'),
    State('custom_execute_export_data_1','data'))
def update_dashboard_message_board_18_1(n_clicks,already_out):
    if n_clicks > 0:
        children_list = []
        _val = 'Started Custom Executing Query'
        children_list.append(html.Div([html.Iframe(id="custom_execute_msg",
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
                                                                        math.ceil(len(_val) / 170) * 25)}
...
@app.callback(
            [Output('custom_execute', 'children'),
            Output('custom_execute_export_data','data')],
            Input('custom_execute_query', 'n_clicks'),
            State('custom_sttm_file','data'),
            State('config_input','data'),
            State('custom_execute_export_data','data'))
def update_dashboard_message_board_18_1(n_clicks,file,config_dict,already_out):
    ### Add Custom testing Function Here ######
    ...
    if n_clicks > 0:
        ...
        run_time = datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S").replace(" ", '-').replace(':',
            ...
            '/', '')
        conn = dbricks.DatabricksConnection.getConnection('', config_dict['Databricks connection string'])
        custom_etl_obj = CSTM_ETL.ValidateData('conn', config_dict['Output folder path'], run_time)
        _val, path = custom_etl_obj.GenerateReport()
        _val = _val + ' Custom ETL Validation Completed Successfully %s' % path
        children_list.append(html.Div([html.Iframe(id="strategy_validation_msg_func",
                                                    srcDoc=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} : {_val}',
                                                    style={'border': 0, 'width': '100%',
                                                            'height': (len(_val.split('<BR>')) * 15) + (
    ...
    if n_clicks > 0:
        if selection_list != None:
            if 'Test Case Generation' in selection_list:
                ...
                print(children_list,already_out)
        ...
        if selection_list != None:
            if 'Lake - Lake Hist Query Generation' in selection_list:
                ...
                run_time = datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S").replace(" ", '-').replace(':',
                    ...
                    '/', '')
                sttm_data = rdconfig.ReadConfigData.ReadSttmData("",
                                                                 config_data['Input folder path'] + '\\Input Files\\' +
                                                                 config_data['Lake to Lake Hist STTM name'])
                query_gen = QG.QueryGenerator(sttm_data, config_data['Output folder path'], run_time)
                _val = query_gen.generate_queries(config_data['Input folder path'] + '\\Pyfiles\\Query_tempelate.xlsx',
                                                  'Lake-LakeHist')
    ...
    if n_clicks > 0:
        if selection_list != None:
            if 'Hub to Mart Query Generataion' in selection_list:
                ...
                sttm_data = rdconfig.ReadConfigData.ReadSttmData("",
                                                                 config_data['Input folder path'] + '\\Input Files\\' +
                                                                 config_data['Hub to Mart STTM name'])
                query_gen = QG.QueryGenerator(sttm_data, config_data['Output folder path'], run_time)
                _val = query_gen.generate_queries(
        ...
        if selection_list != None:
            if 'Source - Raw Data Validation' in selection_list:
                ...
                for file_num in range(len(config_data['Raw data file name'])):
                    ...
                    config_data['Input folder path'], config_data['Source data file name'][file_num]),
                                              '%s\\Input Files\\%s' % (config_data['Input folder path'],
                                                                       config_data['Raw data file name'][file_num]),
                                              False, run_time)
            ...
            if 'Source - Lake Data Validtion' in selection_list:
                ...
                for file_num in range(len(config_data['Raw data file name'])):
                    ...
                    _val = src_lake_obj.srctolake(
                        ...
                        config_data['Input folder path'], config_data['Source data file name'][file_num]),
                        '%s\\Input Files\\%s' % (
                        config_data['Input folder path'], config_data['Raw data file name'][file_num]),
                        config_data['Lake table name'][file_num], run_time)
